...
export default function Layout() {
  ...
  const t = {
    ...
    navIdle:    isDark ? 'text-slate-400 hover:text-slate-100 hover:bg-slate-800'
    ...
    toggleBg:   isDark ? 'bg-slate-700 hover:bg-slate-600'
                       : 'bg-gray-100 hover:bg-gray-200',
    toggleText: isDark ? 'text-yellow-400' : 'text-slate-600',
  }
  ...
  return (
    <div className={`flex h-screen ${t.bg} overflow-hidden`}>
      ...
      {/* ── Sidebar ── */}
      <aside className={`w-64 ${t.sidebar} border-r ${t.border} flex flex-col flex-shrink-0`}>
          ...
          {navItems.map(item => (
            <Link key={item.path} to={item.path}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm
                ...
                location.pathname === item.path ? t.navActive : t.navIdle
              }`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Top bar */}
        <header className={`h-14 border-b ${t.border} px-6 flex items-center
                            justify-between ${t.headerBg} flex-shrink-0`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={toggle}
              className={`w-9 h-9 rounded-lg flex items-center justify-center
                          transition-colors ${t.toggleBg} ${t.toggleText}`}
